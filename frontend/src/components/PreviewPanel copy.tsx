...
const PreviewPanel: React.FC<PreviewPanelProps> = ({
  ...
  const handleSubmit = async (prompt?: string) => {
    ...
    // Build payload here, including prompt
    const payload = {
      uid: activeUserId,
      preview: initialAsset,
      gallery: doneFiles
        .map(f => safeParse((f as any).response))
        .filter(Boolean),
      prompt,  // <— added
    };
  ...
  const handleGenerate = React.useCallback((prompt: string) => {
    // you decide what to do here:
    // - maybe include `prompt` in payload
    // - or just call handleSubmit()
    console.log("Prompt from GenerateCard:", prompt);
    handleSubmit(prompt);      // or whatever parent logic you want
  }, [handleSubmit]);
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "center", }}>
          <GenerateCard uId={activeUserId} fileList={galleryFiles} onFileListChange={setGalleryFiles} onGenerateClick={handleGenerate} isLoading={isLoading} />
        </div>
      </div>
